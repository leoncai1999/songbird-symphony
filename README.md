# Songbird Symphony

<img width="374" height="682" alt="Screenshot 2026-05-19 at 10 55 56 PM" src="https://github.com/user-attachments/assets/a0c7f7b3-3fc0-41d5-8be5-2bc3db3cc12f" />

Songbird Symphony is a web experience that allows you to create your own chorus of bird sounds. Simply click on a bird card to begin playing that bird's sound. You can select as many bird sounds as you like. You can stop the sound of a bird by tapping the bird card again or waiting for the recording to run its course. Additionally, the reset button will allow you to stop all bird sounds.

## Technical Details

The project design can be viewed on Figma [here](https://www.figma.com/design/i2BYcrQQY1ltc3INxzX2pa/Songbird-Symphony?node-id=0-1&t=5YFKWGLkmEZFvPiN-1). This web app was built with React and deployed on Netlify. The domain name was acquired from Namecheap.
